...
!$OMP SINGLE
          if(nrtm > 0) then
            if(allocated(inter_struct%voxel_nodes) .and. inter_struct%voxel_size < res8) deallocate(inter_struct%voxel_nodes)
            ...
            if(.not.allocated(inter_struct%voxel_nodes)) then
!             call my_alloc(inter_struct%voxel,res8)
              ALLOCATE(inter_struct%voxel_nodes(res8))
            ...
            do i=1,inter_struct%voxel_size
              inter_struct%voxel_nodes(i)%nb=0
