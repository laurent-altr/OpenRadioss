...
#pragma omp critical
...

    {
        ...
        *i_mem = 0;
        ...
        size_t nbx_fine = static_cast<size_t>(1 * (nbx + 2));
        size_t nby_fine = static_cast<size_t>(1 * (nby + 2));
        size_t nbz_fine = static_cast<size_t>(1 * (nbz + 2));
...
#pragma omp for schedule(dynamic)
        for (int ne = 0; ne < nrtm; ++ne)
        {
            ...
            for (size_t iz = iz1; iz <= iz2; ++iz)
            {
                ...
                {
                    const std::vector<int> nodes = voxelGrid.getChunk(ix1, ix2, iy, iz);
                    for (const auto &jj : nodes)
                    {
