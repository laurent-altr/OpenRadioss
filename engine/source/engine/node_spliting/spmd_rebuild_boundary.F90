...
!! \param[out] boundary_size    total number of entries in boundary
        ...
        subroutine spmd_rebuild_boundary(numnod, nspmd, ispmd, main_proc, itab, &
          ...
          allocate(boundary_add(2, nspmd + 1))
          boundary_add(1, 1) = 1
          do j = 1, nspmd
            boundary_add(2, j)     = boundary_add(1, j) + recv_cnt(j)
            boundary_add(1, j + 1) = boundary_add(2, j) + send_cnt(j)
          end do
          boundary_add(2, nspmd + 1) = boundary_add(1, nspmd + 1)  ! sentinel
