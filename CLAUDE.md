...
```bash
./build_script.sh -arch=linux64_gf -debug=asan -mpi=ompi
```
...
Every source file must begin with the standard copyright block (see `template/template.F90`). The `scripts/headers.py` script updates headers automatically. Use the script to create the header.
...
- Code analysis uses a custom GCC plugin at `scripts/gcc_plugin/` (build with `scripts/gcc_plugin/build.sh`) : use -debug=analysis (look at the tail of the log ) for some code static analysis. 
